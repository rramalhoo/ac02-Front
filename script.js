...
function cadastrar(produto, quantidade) {
    if (quantidade !== -1) {
        produtos.push(produto.toLowerCase())
        quantidades.push(quantidade)
    } else {
        totalprod = produtos.length
        quantestoque = 0
        quantidades.forEach(quant => {
            quantestoque += quant
        })
        ...
        prodconsult = produto.toLowerCase()
        quantconsult = 0
        for (let i = 0; i < produtos.length; i++) {
            if (produtos[i] === prodconsult) {
                quantconsult = quantidades[i]
            }
        }
        ...
        resp = `
            ...
            <p>
                <b>Total de produtos cadastrados:</b> ${totalprod} | 
                <b>Quantidade total em estoque:</b> ${quantestoque} | 
                <b>Produto mais frequente:</b> ${prodmf} | 
                <b>Produto específico (${prodconsult}):</b> Quantidade ${quantconsult}
            </p>
        `
